...
(function () {
  ...
  async function handleFile(file) {
    ...
    try {
      ...
      const classification = await classifyWithModel(file, metadata, summary);
      ...
      const fallback = await classifyWithModel(file, {}, {});
  ...
  async function analyzeFile(file) {
    ...
    const verdict = await classifyWithModel(file, metadata, summary);
    return { metadata, summary, verdict };
  }

  async function classifyWithModel(file, metadata, summary) {
    const heuristic = classify(metadata, summary);
    const model = await runModelInference(file).catch(() => null);
    if (!model) return heuristic;
    return modelToVerdict(heuristic, model);
  }
  ...
  async function runModelInference(file) {
    ...
    const apiBaseUrl = (window.AI_DETECTOR_API_URL || DEFAULT_API_URL).replace(/\/+$/, '');
    ...
    const modelCredibility = clampPercent(payload?.analysis?.score);
    const modelAiLikelihood = clampPercent(100 - modelCredibility);
    return {
      raw: payload,
      detectionResult: payload?.analysis?.status || 'Analysis complete',
      confidenceScore: modelCredibility,
      credibilityScore: modelCredibility,
      aiLikelihood: modelAiLikelihood,
      modelCredibility,
      modelAiLikelihood,
      modelStatus: String(payload?.analysis?.status || ''),
    };
  }

  function modelToVerdict(heuristic, model) {
    const isAi = model.aiLikelihood >= 50;
    const confidence = clampPercent(isAi ? model.aiLikelihood : 100 - model.aiLikelihood);
    const reasons = [...(heuristic.reasons || [])];
    reasons.unshift(`Model status: ${model.modelStatus || 'Unknown'} (AI likelihood ${model.aiLikelihood.toFixed(2)}%).`);
    reasons.unshift('Final verdict uses model inference as the primary decision source.');
    const breakdown = Array.isArray(heuristic.breakdown) ? [...heuristic.breakdown] : [];
    breakdown.unshift({ label: 'Model AI likelihood', weight: model.aiLikelihood / 100, sign: 1 });
    return {
      ...heuristic,
      isAi,
      confidence,
      ...
      breakdown,
      model,
      blendedAiLikelihood: model.aiLikelihood,
    };
  }
